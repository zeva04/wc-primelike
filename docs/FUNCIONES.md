...
| `applyFiloExecution(run,match)` | **philosophy** (Bible §5 "successful execution"): convierte `match.filoHits` (aciertos de actos del tipo firma — Y de la avanzada desde M2 — cuenta el Match vía `noteFiloHit`) en progreso de la arista firma: +`FILO_EXEC_GAIN` (0.25) por acierto, tope `FILO_EXEC_CAP` (2) por partido. La llama `flow.postMatchUpdate`; devuelve `{arista, add, hits}` o `null`. |
...
| `identityGapMult(oppTeam, myNivel, koRound)` | **philosophy** (R3): la BRECHA DE IDENTIDAD — ×(1 + `IDENTITY_GAP_PCT`·brecha) con brecha = nivel rival madurado − mi nivel (piso 0: solo castiga). +4%/nivel, se apila sobre `p.forma` en el Match; ×1 en grupos; el Consolidado es inmune. El informe la narra (`modoMundial.brechaPct`). |
