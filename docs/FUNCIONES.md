...
| `filoShareShift(myFilo,oppFilo)` / `filoRasgo(m,filoId)` | **match/sequences** (F2): cuánto inclina la filosofía el reparto de iniciativa (mi Contra −0.05 · mi Bloque −0.08 · rival contra +0.04 · rival bloque +0.06, puro, se suma al mineShare; el Bloque además lleva balón parado ×1.3 como arma propia — ajuste PO post-gate) / ¿tengo el rasgo de Consolidada activo? (nivel 2). Los 4 rasgos: Press +0.05 al robo total · Posesión +1 acto de circulación (plan propio `s.plan`) · Contra +0.04 al perfil de transición (también en la conversión def→of) · Bloque +0.05 a la contención del repliegue. |
...
| `filoCard()` (hub) / `renderPhilosophy()` (pantalla `philosophy`) | **F3 "La vitrina"**: la card compacta de identidad del hub (filosofía, nivel, barra al próximo umbral; clic → pantalla) / la pantalla completa: nivel con barra, las 5 aristas (propias destacadas, cada una con el tipo de jugada que genera), la firma con su ×mult, el rasgo (🔒 hasta Consolidada) y MI fila de la matriz en cualitativo (regla 4). Solo lectura: el cambio sigue en el hub. |
