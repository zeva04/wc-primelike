...
| `currentLineup(squad, prev, id)` | **Puerta de entrada de las pantallas** (hub y squad): devuelve `{lineup, formationId}` ya ordenado por slots y con los puestos asignados; rearma el once si una baja lo invalidó. El smoke la usa también. Si la formación pedida ya no se puede armar, deriva una del once que quedó — **pero solo si son 6**: con 5 (plantel sin arquero) el label de `formationLabel` es ambiguo y adoptarlo castigaba a tres jugadores fuera de puesto (ver CORE §Plantel diezmado). |
...
| `formationLabel(selected)` | Etiqueta de formación del once (ej. `2-1-2`). **Cuenta solo DEF/MED/DEL y da por sentado el arquero**: en un once de 5 sin POR devuelve una etiqueta que suma 5 y colisiona con la de una formación real de 6 — por eso `currentLineup` solo la adopta con el once completo. |
...
| `buildOpponentReport(run, oppId)` | **scouting**: el Informe del Rival (Bible §4.6) — `{lineas: {ataque\|defensa\|arquero: {nivel, detalle}}, figura, forma, bajas, enEliminatorias, modoMundial}`. Niveles CUALITATIVOS (Alto/Medio/Bajo) comparando el cruce real (su ataque vs tu defensa, su defensa vs tu ataque, arquero vs arquero) con poderes esperados sin buffs; forma desde los resultados jugados por el mundo vivo (máx 3, reciente primero); bajas de `run.rivalBans`. **`bloque` (sprint del Territorio)**: con qué altura se va a parar el rival (`field.baseHeight` — la MISMA fuente que usará el partido) y qué camino deja abierto, en palabras: es lo que le faltaba a la decisión de altura del DT, que hasta entonces se tomaba a ciegas. **`modoMundial` (sprint de la Escalada)**: la escalada de la ronda dicha por un ojeador — `{ronda, titulo, texto, madura, brecha}`, sin un solo porcentaje (era la única excepción a la regla cualitativa del módulo; `tests/escalada.test` la prohíbe). PURO: no muta la run ni consume rng — mirar es gratis (los tests de pureza en `tests/scouting.test.js`). |
...
| Función | Qué hace |
|---|---|
| `quickSim(idA,idB,knockout)` | Simula por Poisson un partido que el usuario no juega; en eliminatoria resuelve prórroga y penales. **Sprint de la Escalada**: el rating pesa MUCHO más en KO (`SPREAD_KO` 0.85) que en grupos (`SPREAD_GROUPS` 0.55) — el mundo casi no seleccionaba y el 6% de las finales salían contra un rival ≤69 (hoy 2%). Los grupos conservan su caos a propósito: el batacazo es el combustible del Daily. La prórroga y la tanda NO escalan (si el grande llegó empatado, ya falló). |
...
| `tickField(m,mine,opp)` | **Un minuto de territorio, sin azar**: de quién es la pelota (Bresenham por bloques de 3'), el RECORRIDO de esa posesión (nace atrás, muere adelante — el target es el centro de gravedad, no el sitio), el carril por amplitud, el calor del minuto y el sobrecosto físico del bloque adelantado. Lo llama `Match.tick` **antes** que las jugadas. |
| `matchPush(m)` | **Cuánto empuja el PARTIDO el territorio**, en alturas: el dominio en vivo (`momentumTrend`, saturado en 14 — medido) más la urgencia del marcador pesada por el reloj, **asimétrica** (el que pierde se vuelca; el que gana administra sin esconderse), más ±0.5 por hombre de rojas. Es el espejo de `oppReaction` para mi lado, y lo que hace que una goleada se vea en el mapa **por acumulación**. Puro y sin azar. |
| `ballZone(m)` / `setBall(m,{h,v,side})` / `moveBall(m,dv,dh)` | Leer y mover la pelota. Mover deja calor de jugada real (`HEAT_ACT` = 5 vs `HEAT_TICK` = 1: el mapa contesta *dónde se jugó el partido*, no *dónde estuvo la pelota*). |
...
| `otherLane(m)` / `wingLane(m)` / `inWing(m)` | El carril opuesto (el cambio de frente), un carril de banda alternando sin gastar azar, y si la pelota está por afuera. `wingLane` comparte contador con la deriva: las dos fuentes de banda se turnan, así un partido no carga siempre el mismo lado. |
...
| `previewDayAction(run,actionId)` | **day-action**: PROYECTA una acción sin ejecutarla. Devuelve `{...accion, mult, sim}` donde `sim` es el run **tal como quedaría** al confirmar — clonado (`structuredClone`) y pasado por el `effect` real de la acción más `trackOxidacion`. Nadie reimplementa la fórmula, así que los clamps y el multiplicador del día entran solos; `sim` es de solo lectura y descartable. La usa la hoja de confirmación del hub para pintar el antes/después, y `day-action.test` fija la igualdad **preview == aplicar**. `null` en los mismos casos que `applyDayAction` (sin acción pendiente, id desconocido, bloqueada hoy). **Solo para `DAY_ACTIONS`**: los efectos de las Oportunidades pueden consumir azar y simular uno desplazaría la secuencia del `rng` global. |
...
| `tourneyFormaMult(koRound)` | **opponents** (R2 · curva rehecha en el sprint de la Escalada): la FORMA DE TORNEO, leída de la tabla **CONVEXA** `TOURNEY_FORM` = ×1.03 · ×1.10 · ×1.19 · ×1.31 · **×1.45**. Nació lineal (+3%/ronda, final ×1.15) y la progresión compuesta del DT le ganaba la carrera. `genOpponentLineup(team, banned, koRound)` la estampa como `p.forma` en el once rival; solo MIS partidos (el mundo simulado no cambia). Ver CORE §La Escalada. |
...
| `hub/team.js` | Lo que el DT **ajusta** de su equipo: el canje de entrenamiento y la **altura del bloque** (el picker que comparten el día de partido y el Informe). |
| `hub/day.js` | **El día**: la portada del Daily y los modales que abre pasar de día (evento inevitable, conflicto con decisión). |
...
| `medidoresEquipo(run)` / `medidor(m)` (`hub/hud`) | **La fuente única de los tres medidores** del equipo (Moral, Energía media, Ritmo) y su pintado en 16 casillas: `[{id, ico, label, pct, num, valor, color}]`. Los usa el panel "Mi equipo" y **los vuelve a usar la hoja de confirmación**, que muestra el antes y el después con las mismas barras — calculados en dos sitios, la promesa de la hoja y el estado del panel podrían decir cosas distintas. `num` es el número escribible ("56% → 71%"); el Ritmo va en `null` porque su barra es una escala de tres escalones y ahí lo que se compara es el texto. |
| `hojaPrevia(estado)` / `hojaResultado(estado)` / `animarHoja()` / `fotoAntes(id)` (`hub/confirm`) | **La hoja de confirmación de la Acción del Día**, en sus dos caras y el mismo sitio. La PREVIA lee `previewDayAction` y pinta lo que va a cambiar: el boost de stat del entrenamiento, el cambio de identidad del Plan, y una barra por medidor que se mueva — los cuadrados del tramo afectado **titilan** (contorno en el color de su destino; rojo si son los que se pierden) y debajo va el `56% → 71%`. Sale con Cancelar (no toca el run) o Confirmar. El RESULTADO se pinta desde `fotoAntes` —la foto tomada con el run todavía intacto— y `animarHoja` suelta los cuadrados a su valor final en cascada (`transition-delay` escalonado). El viaje es CSS y no un temporizador: el congelador del deep-link lo deja directamente en el final, y una captura sale limpia. `animarHoja` va en dos tiempos (apagar el titileo → `offsetWidth` → escribir el destino) porque animación y transición en el mismo ciclo no dejan nada desde donde animar; no usa `requestAnimationFrame` porque el rAF no dispara si la pestaña no está componiendo. |
...
- **`smoke.js`** — simula runs completas sin UI con decisiones al azar y verifica invariantes
  ...
  deriva de balance; `--all` tabula las 23 jugables. Imprime la **CURVA** (sprint de la Escalada): de los que LLEGAN a cada ronda KO, cuántos la ganan, y el **salto** 16avos→FINAL. Es el gate de dificultad del proyecto — el % de campeón cae por acumular cinco victorias, no porque el torneo se endurezca (ver CORE §La Escalada).
...
- **`escalada.test.js`** (sprint de la Escalada) — el contrato de la dificultad por ronda:
  la **FORMA** de `TOURNEY_FORM` (convexa — cada ronda sube MÁS que la anterior; un escalón
  plano en el medio devuelve el problema original), los 16avos clavados en ×1.03 y la final
  en ×1.45, `koRound` fuera de rango sin `NaN`; que `quickSim` pese el rating mucho más en KO
  que en grupos **sin matar el caos de grupos** ni convertir un cruce parejo en jerarquía;
  que el bracket simulado deje vivos a equipos cada vez mejores; y que el Modo Mundial del
  informe **no exponga ninguna magnitud numérica** ni repita texto entre rondas.
